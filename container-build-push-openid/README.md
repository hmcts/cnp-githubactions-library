...
A composite GitHub Action that builds and pushes container images to Azure Container Registry (ACR) with built-in OpenID Connect (OIDC) authentication and multi-platform support.

> **Note:** This action handles OIDC authentication internally. Simply provide your Azure client ID and tenant ID, and the action will authenticate with ACR automatically.
...
- Designed for OIDC-authenticated registries
...
steps:
  ...
  - name: Scan container image
    run: |
      trivy image myacrname.azurecr.io/my-app:latest
```

### Custom Workflow with Additional Logic

```yaml
name: Custom Container Build with OIDC
  ...
  build:
    ...
    steps:
      ...
      - name: Deploy to staging
        run: |
          kubectl set image deployment/my-app my-app=myacrname.azurecr.io/my-app:v${{ steps.version.outputs.version }}
...
```yaml
name: Multi-Environment Build with OIDC
    ...
    steps:
      ...
      - name: Determine environment
        ...
        run: |
          if [[ "${{ github.ref }}" == "refs/heads/main" ]]; then
            ...
            echo "acr_name=hmctsprod" >> $GITHUB_OUTPUT
          else
            echo "environment=development" >> $GITHUB_OUTPUT
            echo "dockerfile=Dockerfile.dev" >> $GITHUB_OUTPUT
            echo "acr_name=hmctspublic" >> $GITHUB_OUTPUT
          fi
      ...
      - name: Build and push
        ...
        with:
          azure-client-id: ${{ secrets.AZURE_CLIENT_ID }}
          azure-tenant-id: ${{ secrets.AZURE_TENANT_ID }}
          registry-name: ${{ steps.env.outputs.acr_name }}
          image-name: my-app
      ...
      - name: Notify deployment
        run: |
          curl -X POST ${{ secrets.WEBHOOK_URL }} \
            ...
            -d '{"environment": "${{ steps.env.outputs.environment }}", "image": "${{ steps.env.outputs.acr_name }}.azurecr.io/my-app:${{ github.sha }}"}'
```

### Using with Matrix Strategy

```yaml
name: Multi-App Build with OIDC
...
| `registry-name` | Azure Container Registry name (without .azurecr.io) | **Yes** | - |
...
- name: Build with custom cache
  ...
  with:
    ...
    image-name: my-app
    cache-from: |
      type=registry,ref=myacrname.azurecr.io/my-app:buildcache
      type=gha
    cache-to: type=registry,ref=myacrname.azurecr.io/my-app:buildcache,mode=max
```
...
- name: Sign image
  run: |
    cosign sign --key cosign.key myacrname.azurecr.io/my-app@${{ steps.build.outputs.digest }}

- name: Create SBOM
  run: |
    syft myacrname.azurecr.io/my-app@${{ steps.build.outputs.digest }} -o spdx-json > sbom.json
```
...
- You want to add custom steps before or after the build
- You want to integrate with other actions in the same job
- You need to use outputs from previous steps
- You're building multiple images in one job (matrix strategy)
- You need environment-specific build logic

**Use the Reusable Workflow when:**
- You want a simple, one-line build process
...
- **OIDC Authentication:** This action handles all OIDC authentication internally
  ...
  - ACR login is performed automatically after Azure authentication
  - No need for external authentication setup
- Registry-username and registry-password inputs are not supported; use OIDC instead
- The action expects the GitHub Actions job to have `permissions: { id-token: write }` for OIDC token generation
